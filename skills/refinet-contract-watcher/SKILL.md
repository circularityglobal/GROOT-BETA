...
|---|---|---|
| Chain watchers | SQLite `public.db` | `chain_watchers` table |
| Captured events | SQLite `public.db` | `chain_events` table (columns: `chain`, `received_at`) |
| Registry ABIs | SQLite `public.db` | `registry_abis` table |
| Parsed functions | SQLite `public.db` | via `execution_logic` table |
| Parsed events | SQLite `public.db` | via ABI JSON in `registry_abis.abi_json` |
...
def get_watched_contracts(db) -> list[dict]:
    ...
    return db.execute("""
        SELECT DISTINCT ra.contract_address, ra.chain, rp.name as project_name,
               COUNT(DISTINCT rs.user_id) as star_count,
               COUNT(DISTINCT rf.forked_by) as fork_count
        FROM registry_abis ra
        JOIN registry_projects rp ON rp.id = ra.project_id
        LEFT JOIN registry_stars rs ON rs.project_id = rp.id
        LEFT JOIN registry_forks rf ON rf.source_project_id = rp.id
        GROUP BY ra.contract_address, ra.chain
        HAVING star_count > 0 OR fork_count > 0
        ORDER BY star_count + fork_count DESC
    """).fetchall()


def check_contract_activity(contract_address: str, chain: str) -> dict:
...
def correlate_bridge_events(events: list[dict]) -> list[dict]:
    ...
    for event in events:
        ...
        for bridge_name, addresses in KNOWN_BRIDGE_CONTRACTS.items():
            for chain_id, addr in addresses.items():
                if contract == addr.lower() and event.get("chain") == chain_id:
    ...
    for bridge_name, bevents in bridge_events.items():
        l1_events = [e for e in bevents if e["chain"] == 1]
        l2_events = [e for e in bevents if e["chain"] != 1]
...
schedules:
  ...
  - name: abi-security-scan
    ...
    agent: contract-watcher
    task: >
      Check registry_abis table for ABIs uploaded since last scan that
