...
def get_stats(db) -> dict:
    ...
    stats["new_documents_24h"] = db.execute(
        "SELECT COUNT(*) FROM knowledge_documents WHERE created_at > ?", (yesterday,)
    ).fetchone()[0]

    stats["new_chunks_24h"] = db.execute(
        "SELECT COUNT(*) FROM knowledge_chunks WHERE created_at > ?", (yesterday,)
    ).fetchone()[0]
    ...
    try:
        ...
        stats["cag_functions"] = db.execute(
            "SELECT COUNT(*) FROM contract_definitions WHERE abi_json IS NOT NULL"
        ).fetchone()[0]
...
def prune_stale(db, stale_chunks: list[dict]) -> int:
    ...
    db.execute(f"DELETE FROM knowledge_chunks WHERE id IN ({ph})", chunk_ids)
    db.commit()
