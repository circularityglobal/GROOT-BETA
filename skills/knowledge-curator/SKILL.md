...
|---|---|---|
| Document metadata | SQLite `public.db` | `documents` table |
| Document chunks | SQLite `public.db` | `document_chunks` table |
| Chunk embeddings | SQLite `public.db` | `chunk_embeddings` table (384-dim float vectors) |
| Full-text index | SQLite FTS5 | `document_chunks_fts` virtual table |
| Contract ABIs | SQLite `public.db` | `contract_abis` table |
| Parsed SDK defs | SQLite `public.db` | `contract_sdks` table |
| CAG search index | SQLite `public.db` | `cag_index` table (function sigs + embeddings) |
...
def verify_ingestion(document_id: str) -> dict:
    ...
    doc = db.execute(
        "SELECT id, title, format, visibility, created_at FROM documents WHERE id = ?",
        (document_id,)
    ).fetchone()
    ...
    if embedded_count > 0:
        sample = db.execute(
            "SELECT embedding FROM chunk_embeddings WHERE chunk_id IN "
            "(SELECT id FROM document_chunks WHERE document_id = ?) LIMIT 1",
            (document_id,)
        ).fetchone()
        if sample:
            import json
...
def reembed_document(document_id: str):
    ...
    # 1. Delete existing chunks and embeddings for this document
    db.execute("DELETE FROM chunk_embeddings WHERE chunk_id IN "
               "(SELECT id FROM document_chunks WHERE document_id = ?)",
               (document_id,))
    db.execute("DELETE FROM document_chunks WHERE document_id = ?",
               (document_id,))
    db.commit()
...
def prune_stale_chunks() -> dict:
    ...
    # Delete embeddings first (foreign key order)
    db.execute(f"DELETE FROM chunk_embeddings WHERE chunk_id IN ({placeholders})", chunk_ids)
    # Delete chunks
    db.execute(f"DELETE FROM document_chunks WHERE id IN ({placeholders})", chunk_ids)
    db.commit()
...
def sync_cag_index() -> dict:
    ...
    unindexed = db.execute("""
        SELECT ca.id, ca.project_id, ca.chain_id, ca.contract_address
...
5. Insert into `document_chunks`, `chunk_embeddings`, `document_chunks_fts`
