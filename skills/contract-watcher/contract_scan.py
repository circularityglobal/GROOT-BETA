...
def get_unscanned_abis(db) -> list[dict]:
    ...
    try:
        rows = db.execute("""
            SELECT ca.id, ca.project_id, ca.chain_id, ca.contract_address, ca.name,
        ...
        """).fetchall()
        return [
            {"abi_id": r[0], "project_id": r[1], "chain_id": r[2],
...
def analyze_abi_from_db(db, abi_id: str) -> dict:
    ...
    row = db.execute(
        "SELECT abi_json FROM contract_abis WHERE id = ?", (abi_id,)
    ).fetchone()
    if not row:
...
def get_event_stats(db) -> dict:
    ...
    try:
        stats["total_listeners"] = db.execute(
            "SELECT COUNT(*) FROM chain_listeners WHERE is_active = 1"
        ).fetchone()[0]
        stats["total_events"] = db.execute(
            "SELECT COUNT(*) FROM chain_events"
        ).fetchone()[0]
        stats["unprocessed_events"] = db.execute(
            "SELECT COUNT(*) FROM chain_events WHERE status = 'raw'"
        ).fetchone()[0]

        week_ago = (datetime.now(timezone.utc) - timedelta(days=7)).isoformat()
        stats["events_this_week"] = db.execute(
            "SELECT COUNT(*) FROM chain_events WHERE created_at > ?", (week_ago,)
        ).fetchone()[0]

        # Events by chain
        chains = db.execute(
            "SELECT chain_id, COUNT(*) FROM chain_events GROUP BY chain_id"
        ).fetchall()
